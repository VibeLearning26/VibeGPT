...
class DocumentProcessingService:
    ...
    async def process_document(self, job: DocumentProcessingJob) -> None:
        ...
        try:
            ...
            await self._update_job_status(
                job.id, ProcessingJobStatus.COMPLETED, completed=True, chunks_created=len(chunks)
            )
    ...
    async def _get_document(self, doc_id) -> Document | None:
    ...
    async def _update_doc_status(self, doc_id, status: DocumentStatus) -> None:
        await self.db.execute(update(Document).where(Document.id == doc_id).values(status=status))
        await self.db.flush()
    ...
    async def _update_job_status(
        ...
        await self.db.execute(
            update(DocumentProcessingJob).where(DocumentProcessingJob.id == job_id).values(**values)
        )
        await self.db.flush()
